...
public class Workspace extends AppWorkspaceComponent {
    ...
    static final String CLASS_REMOVE_BUTTON = "remove_button";
	...
	removeButton.setPrefWidth(BUTTON_TAG_WIDTH);
        ...
        removeButton.setOnAction(e -> {
            String tagName = removeButton.getText();
            //System.out.println("Clicked RemoveButton :" + tagName);
            //HTMLTagPrototype clickedTag = dataManager.getTag(tagName);
            //System.out.println("clickedTag" + clickedTag);
	...
	for (HTMLTagPrototype tag : dataManager.getTags()) {
	    ...
	    Button tagButton = new Button(tag.getTagName());
            tagButton.setId(tag.getTagName());
	...
	fileManager.exportData(dataManager, TEMP_PAGE);
       // System.out.println("requesting exportData from Workspace - 251 line");
	loadTempPage();
    }
	...
	for (Button b : tagButtons) {
        ...
        tagToolbar.getChildren().get(0).getStyleClass().add(CLASS_REMOVE_BUTTON);
	...
	try {
		...
		Collection<String> keys = attributes.keySet();
                ...
                boolean[] buttonNums = new boolean[12];
                int buttonNum = 0;
                
                //CHEAK TO ENABLE/DISABLE BUTTONS IF USER CLICK A TAG ON THE TAG TREE
                for (HTMLTagPrototype tagg : dataManager2.getTags()) {
                   buttonNum++;
                   
                    for(int i=0; i<tagg.getLegalParents().size(); i++){
                        if(tagg.getLegalParents().get(i).equals(selectedTag.getTagName())){
                            //tagButtons.get(buttonNum-1).setDisable(false);
                            buttonNums[buttonNum-1] = true;
                            //System.out.println(buttonNum-1+ " allowed tagName:"+tagg.getTagName() +" selectedTag: "+ selectedTag.getTagName());
                        }
                    }
                }
                    
                for(int i=0; i<tagButtons.size(); i++){
                    if(buttonNums[i]==true)
                ...
                if(checkName.equals("html") || checkName.equals("head") 
                    ...
                    tagToolbar.getChildren().get(0).setDisable(true);
                }else
                    tagToolbar.getChildren().get(0).setDisable(false);
	    ...
	    FileManager fileManager = (FileManager) app.getFileComponent();
	    fileManager.exportData(dataManager, TEMP_PAGE);
            fileManager.exportCSS(dataManager.getCSSText(), TEMP_CSS_PATH);
            //System.out.println("[requesting from reloadWorkspace() in Workspace to FileManager - 369 line]");
